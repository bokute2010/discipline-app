...
const CrudAvatarUpload = () => {
  const { avatarInput, changeAvatar, avatar } = useUserProfile();

  const avatarDefault = avatar || toAbsoluteUrl(`/media/avatars/avatars/300-2.png`);

  return (
    <ImageInput value={avatarInput} onChange={changeAvatar}>
      {({ onImageUpload }) => (
        <div className="image-input size-16" onClick={onImageUpload}>
          ...
          >
            <img
              src={avatarInput.length > 0 ? avatarInput[0].dataURL : avatarDefault}
              alt="avatar"
